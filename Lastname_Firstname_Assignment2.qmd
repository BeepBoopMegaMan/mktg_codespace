...
customer_basic = email.select(["ID", "age", "avg_distance"])
print(customer_basic)
...
customers_30_and_above = email.filter(
  pl.col("age") >= 30
)

print(customers_30_and_above)
...
new_dataset = email.select(
  (pl.mean("n_purchase")).alias("mean_n_purchase"),
  (pl.min("age")).alias("min_age"),
  (pl.sum("total_revenue")).alias("sum_total_revenue")
)
...
email.group_by("success").agg(
  (pl.mean("age")).alias("average_age"),
  (pl.max("n_purchase")).alias("max_n_purchase"),
  (pl.median("total_revenue")).alias("median_total_revenue")
)
