...
observation = (
    observation_data.group_by("Customer.ID").agg
    (
        pl.col("days_since").min().alias("most_recent_transaction"),
        pl.col("Customer.ID").count().alias
        ("number_of_transactions"),
        pl.col("Total").sum().alias
        ("total_revenue")
    )
)
```
...
customer_data = (
    observation.join(purchaser, on='Customer.ID', how = "left").with_columns(pl.col("made_purchase").fill_null(0))
)
...
# Finish the rest
top_customers = (customer_test.sort("pred_probability", descending= True).head(num_customers_targeted))
actual_purchasers = top_customers.filter(pl.col("made_purchase")==1).shape[0]

ROMI = ((num_customers_targeted*per_purchaser_profit-(num_customers_targeted*per_person_cost))/(num_customers_targeted*per_person_cost)
